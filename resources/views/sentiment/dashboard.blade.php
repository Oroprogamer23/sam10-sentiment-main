                ...
                <div id="results" class="space-y-4 overflow-y-auto max-h-[500px]">
                    @foreach($sentiments as $sentiment)
                        <div class="border-b pb-4">
                            ...
                            <!-- Topic Badge -->
                            <div class="mb-3">
                                <p class="text-sm font-semibold text-gray-600 mb-1">Topic:</p>
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium
                                    {{ $sentiment->topic === 'customer_service' ? 'bg-blue-100 text-blue-800' :
                                       ($sentiment->topic === 'product_quality' ? 'bg-green-100 text-green-800' :
                                       ($sentiment->topic === 'pricing' ? 'bg-purple-100 text-purple-800' :
                                       ($sentiment->topic === 'delivery' ? 'bg-yellow-100 text-yellow-800' :
                                       ($sentiment->topic === 'user_experience' ? 'bg-pink-100 text-pink-800' :
                                        'bg-gray-100 text-gray-800')))) }}">
                                    {{ ucfirst(str_replace('_', ' ', $sentiment->topic ?? 'General')) }}
                                </span>
                            </div>
